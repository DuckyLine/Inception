...
if ! wp user get "${MYSQL_USER2}" --allow-root > /dev/null 2>&1; then
    echo "[i] Creating secondary WordPress user..."
    wp user create \
      "${MYSQL_USER2}" \
      "${MYSQL_MAIL2}" \
      --role=editor \
      --user_pass="${MYSQL_PASSWORD2}" \
      --allow-root
else
    echo "[i] Secondary user '${MYSQL_USER2}' already exists."
fi
